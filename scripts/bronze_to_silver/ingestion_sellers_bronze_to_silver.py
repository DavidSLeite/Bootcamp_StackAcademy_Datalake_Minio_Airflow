...
dag = DAG('ingestion_sellers_bronze_to_silver', 
...
def extract():
    #schema df_customers
    df_customers = pd.DataFrame(data=None, columns=["seller_id","seller_zip_code_prefix","seller_city","seller_state"])

    #lista objetos do bucket
    objects = client.list_objects('bronze', prefix='olist/sellers/',
                                  recursive=True)
    for obj in objects:
    ...
    df_customers.to_csv("/tmp/sellers.csv"
                    ,index=False
                )
...
def load():
    ...
    df_ = pd.read_csv("/tmp/sellers.csv")

    #converte os dados para o formato parquet.
    df_.to_parquet("/tmp/sellers.parquet",index=False)

    #carrega os dados para o Data Lake.
    client.fput_object(
        "silver",
        "olist/sellers/sellers.parquet",
        "/tmp/sellers.parquet"
        )
...
clean_task = BashOperator(
    ...
    bash_command="rm -f /tmp/sellers.csv;rm -f /tmp/sellers.json;rm -f /tmp/sellers.parquet;",
    dag=dag
)
